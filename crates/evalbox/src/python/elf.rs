...
/// Parsed ELF information: needed libraries and search paths in a single pass.
struct ElfInfo {
    needed: Vec<String>,
    rpath: Vec<String>,
    runpath: Vec<String>,
...
/// Parse needed libraries and rpath/runpath from an ELF binary in a single pass.
///
/// Avoids opening, mmapping, and parsing the same file twice (previously
/// `parse_needed` and `parse_rpath_runpath` each did this independently).
fn parse_elf_info(path: &Path) -> Result<ElfInfo, ProbeError> {
    let file = std::fs::File::open(path)?;

    // SAFETY: File is open and valid for the lifetime of the mmap.
